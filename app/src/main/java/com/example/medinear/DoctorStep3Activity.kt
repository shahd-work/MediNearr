...
class DoctorStep3Activity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.btnNext).setOnClickListener {
            ...
            when {
                clinic.isEmpty()    -> Toast.makeText(this, "Please enter clinic name", Toast.LENGTH_SHORT).show()
                address.isEmpty()   -> Toast.makeText(this, "Please enter address", Toast.LENGTH_SHORT).show()
                city.isEmpty()      -> Toast.makeText(this, "Please enter city", Toast.LENGTH_SHORT).show()
                startTime.isEmpty() -> Toast.makeText(this, "Please enter start time", Toast.LENGTH_SHORT).show()
                endTime.isEmpty()   -> Toast.makeText(this, "Please enter end time", Toast.LENGTH_SHORT).show()

                else -> {
                    val intent = Intent(this, DoctorStep4Activity::class.java)

                    // ✅ Forward all data from previous steps
                    intent.putExtra("firstName",      this.intent.getStringExtra("firstName"))
                    intent.putExtra("lastName",       this.intent.getStringExtra("lastName"))
                    ...
                    intent.putExtra("specialization", this.intent.getStringExtra("specialization"))
                    intent.putExtra("license",        this.intent.getStringExtra("license"))
                    intent.putExtra("experience",     this.intent.getStringExtra("experience"))
                    intent.putExtra("fee",            this.intent.getStringExtra("fee"))
                    ...
                    intent.putExtra("slotDuration",  slot)
